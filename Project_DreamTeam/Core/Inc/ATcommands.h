...
/**
 * @brief Write a given command to buffer with the '=?' suffix for testing
 * @note Newline "\r\n" is always appended to the buffer
 *
 * @param buffer ... character buffer which will be written too. Should be big enough to hold AT_CMD_BUFFER_SIZE
 * @param tx ... a callback function, which sends the buffer upon successful buffer write
 * @param cmd ... a command template to use
 * @return length of characters written on success, AT_CMD_ERROR on failure of the underlying printf-functions
 */
int at_test_command(char *buffer, transmit_t tx, char *cmd);

/**
 * @brief Write a given command to buffer with the '?' suffix for querying
 * @note Newline "\r\n" is always appended to the buffer
 *
 * @param buffer ... character buffer which will be written too. Should be big enough to hold AT_CMD_BUFFER_SIZE
 * @param tx ... a callback function, which sends the buffer upon successful buffer write
 * @param cmd ... an AT command template to use
 * @return length of characters written on success, AT_CMD_ERROR on failure of the underlying printf-functions
 */
int at_query_command(char *buffer, transmit_t tx, char *cmd);

/**
 * @brief Write a given command to buffer with the provided parameters as suffix
 * @note Newline "\r\n" is always appended to the buffer
 *
 * @param buffer ... character buffer which will be written too. Should be big enough to hold AT_CMD_BUFFER_SIZE
 * @param tx ... a callback function, which sends the buffer upon successful buffer write
 * @param cmd ... an AT command template to use
 * @param param_format ... a format string for the parameters provided
 * @return length of characters written on success, AT_CMD_ERROR on failure of the underlying printf-functions
 */
int at_set_command(char *buffer, transmit_t tx, char *cmd, const char* param_format, ...);

/**
 * @brief Write a given command to buffer, up to a given limit, with the provided parameters as suffix, limited by
 * @note Newline "\r\n" is always appended to the buffer
 *
 * @param buffer ... character buffer which will be written too. Should be big enough to hold AT_CMD_BUFFER_SIZE
 * @param tx ... a callback function, which sends the buffer upon successful buffer write
 * @param limit ... an upper limit of characters to write. Writing less is possible.
 * @param cmd ... an AT command template to use
 * @param param_format ... a format string for the parameters provided
 * @return length of characters written on success, AT_CMD_ERROR on failure of the underlying printf-functions
 */
...
/**
 * @brief Write a given command to buffer as provided
 * @note Newline "\r\n" is always appended to the buffer
 *
 * @param buffer ... character buffer which will be written too. Should be big enough to hold AT_CMD_BUFFER_SIZE
 * @param tx ... a callback function, which sends the buffer upon successful buffer write
 * @param cmd ... an AT command template to use or a pre-made string
 * @return length of characters written on success, AT_CMD_ERROR on failure of the underlying printf-functions
 */
int at_execute_command(char *buffer, transmit_t tx, char *cmd);

/**
 * @brief handles errors from AT command functions. Serves as an error-handling wrapper for conditions
 * @note It prints "AT Command error.\r\n" on any error
 *
 * @param ret_value ... return value of AT command functions
 * @return true if it matches AT_CMD_ERROR, false otherwise
 */
bool at_handle_error(int ret_value);
